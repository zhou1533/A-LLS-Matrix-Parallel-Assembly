...
__global__ void convertALLStoCSRKernel(
	...
	// Note: 由于现在有多次执行迭代，行清零移到了主机端的 cudaMemset 统一处理，以防覆盖和冲突
	// 详见 compareAssemblyMethods() 
...
PerformanceMetrics compareAssemblyMethods(int num_elements, int total_dofs, int nnz,
	...
	CHECK_CUDA_ERROR(cudaMemcpy(h_color_sizes.data(), d_color_sizes, num_colors * sizeof(int), cudaMemcpyDeviceToHost));

	int max_color_size = 0;
	for (int i = 0; i < num_colors; i++) {
		if (h_color_sizes[i] > max_color_size) max_color_size = h_color_sizes[i];
	}
	int threads_per_block = max(min(max_color_size, 256), 32); // 确保[32, 256]范围内
...
int main() {
	...
	std::cout << "A-LLS method time: " << metrics.alls_time_ms << " ms (Average of " << MEASURE_ITERS << " runs)" << std::endl;
	std::cout << "Colored method time: " << metrics.colored_time_ms << " ms (Average of " << MEASURE_ITERS << " runs)" << std::endl;
