...
struct PerformanceMetrics {
	...
	float speedup; // 染色方法相对于a-COO的加速比
	int atomic_ops_acoo;

	// 新增指标
	size_t acoo_mem_usage;       // a-COO方法GPU内存使用量（字节）
	size_t colored_mem_usage;    // 染色方法GPU内存使用量（字节）
	size_t stiffness_matrix_size; // 刚度矩阵内存大小（字节）
	int num_colors;              // 染色数量
	int max_color_size;          // 最大颜色组大小
	float acoo_flops;            // a-COO方法浮点运算性能（GFLOPS）
...
// ==================== a-COO数据结构管理 ====================
// 预分配的a-COO节点池
__device__ ACOONode* d_acoo_pool = nullptr;
__device__ int* d_acoo_pool_index = nullptr;

// 获取一个新的a-COO节点
__device__ ACOONode* allocateACOONode() {
	int index = atomicAdd(d_acoo_pool_index, 1);
	return &d_acoo_pool[index];
}

// ==================== 基于a-COO的优化核函数 ====================
__global__ void assembleGlobalStiffnessKernelACOO(
	int num_elements,
	const Element* elements,
	ACOONode** acoo_row_heads,
	...
	for (int i = 0; i < ELEM_DOF; i++) {
		...
		for (int j = 0; j < ELEM_DOF; j++) {
			...
			// 获取行头指针
			ACOONode* head = acoo_row_heads[row_global];

			// 查找是否已有该列
			ACOONode* current = head;
			ACOONode* prev = nullptr;
			bool found = false;
			...
			if (!found) {
				// 创建新节点
				ACOONode* newNode = allocateACOONode();
				...
				if (prev == nullptr) {
					// 空链表，使用原子操作设置头指针
					ACOONode* oldHead = (ACOONode*)atomicExch((unsigned long long int*)&acoo_row_heads[row_global],
						(unsigned long long int)newNode);
					newNode->next = oldHead;
				}
				else {
					// 非空链表，使用原子操作插入到prev后面
					ACOONode* oldNext = (ACOONode*)atomicExch((unsigned long long int*)&prev->next,
						(unsigned long long int)newNode);
...
// ==================== 将a-COO转换为CSR ====================
__global__ void convertACOOtoCSRKernel(
	int total_dofs,
	ACOONode** acoo_row_heads,
	double* csr_values,
	...
	// 然后累加a-COO中的值
	ACOONode* current = acoo_row_heads[row];
	while (current != nullptr) {
		int col = current->col;
...
PerformanceMetrics compareAssemblyMethods(
	...
	// 分配原子操作计数器
	int* d_global_atomic_counter_acoo;
	CHECK_CUDA_ERROR(cudaMalloc(&d_global_atomic_counter_acoo, sizeof(int)));

	// 设置网格和块大小
	int threads = BLOCK_SIZE;
	int blocks_acoo = (num_elements + threads - 1) / threads;
	...
	size_t mem_start_acoo = free_mem_start;

	// 测试a-COO方法
	CHECK_CUDA_ERROR(cudaMemset(d_global_atomic_counter_acoo, 0, sizeof(int)));
	CHECK_CUDA_ERROR(cudaMemset(d_csr_values_acoo, 0, nnz * sizeof(double)));

	// 分配a-COO数据结构
	ACOONode** d_acoo_row_heads;
	ACOONode* d_acoo_pool;
	int* d_acoo_pool_index;

	CHECK_CUDA_ERROR(cudaMalloc(&d_acoo_row_heads, total_dofs * sizeof(ACOONode*)));
	CHECK_CUDA_ERROR(cudaMemset(d_acoo_row_heads, 0, total_dofs * sizeof(ACOONode*)));

	// 估计a-COO节点池大小
	int acoo_pool_size = num_elements * ELEM_DOF * ELEM_DOF;
	CHECK_CUDA_ERROR(cudaMalloc(&d_acoo_pool, acoo_pool_size * sizeof(ACOONode)));
	CHECK_CUDA_ERROR(cudaMalloc(&d_acoo_pool_index, sizeof(int)));

	// 初始化a-COO节点池索引
	int h_acoo_pool_index = 0;
	CHECK_CUDA_ERROR(cudaMemcpy(d_acoo_pool_index, &h_acoo_pool_index, sizeof(int), cudaMemcpyHostToDevice));

	// 设置设备全局变量
	CHECK_CUDA_ERROR(cudaMemcpyToSymbol(::d_acoo_pool, &d_acoo_pool, sizeof(ACOONode*)));
	CHECK_CUDA_ERROR(cudaMemcpyToSymbol(::d_acoo_pool_index, &d_acoo_pool_index, sizeof(int*)));

	CHECK_CUDA_ERROR(cudaEventRecord(start));

	// 使用a-COO数据结构组装
	assembleGlobalStiffnessKernelACOO << <blocks_acoo, threads >> > (
		num_elements, d_elements, d_acoo_row_heads, d_global_atomic_counter_acoo);
	CHECK_CUDA_ERROR(cudaDeviceSynchronize());

	// 将a-COO转换为CSR格式
	int blocks_convert = (total_dofs + threads - 1) / threads;
	convertACOOtoCSRKernel << <blocks_convert, threads >> > (
		total_dofs, d_acoo_row_heads, d_csr_values_acoo,
		d_csr_row_ptr, d_csr_col_ind);

	CHECK_CUDA_ERROR(cudaEventRecord(stop));
	CHECK_CUDA_ERROR(cudaDeviceSynchronize());

	CHECK_CUDA_ERROR(cudaEventElapsedTime(&metrics.acoo_time_ms, start, stop));

	// 计算a-COO方法内存使用量
	size_t free_mem_end_acoo;
	CHECK_CUDA_ERROR(cudaMemGetInfo(&free_mem_end_acoo, &total_mem));
	metrics.acoo_mem_usage = mem_start_acoo - free_mem_end_acoo;

	int h_global_atomic_counter_acoo;
	CHECK_CUDA_ERROR(cudaMemcpy(&h_global_atomic_counter_acoo, d_global_atomic_counter_acoo,
		sizeof(int), cudaMemcpyDeviceToHost));
	metrics.atomic_ops_acoo = h_global_atomic_counter_acoo;

	// 估算a-COO方法的浮点运算性能
	// 每个单元的计算量大约为：高斯点(4) * (B矩阵计算 + D矩阵计算 + 刚度矩阵累加)
	metrics.acoo_flops = num_elements * 4 * (1000 + 100 + 200) / 1e9; // 简化估算
	metrics.acoo_flops /= metrics.acoo_time_ms / 1000.0f; // 转换为GFLOPS

	// 清理a-COO数据结构
	CHECK_CUDA_ERROR(cudaFree(d_acoo_row_heads));
	CHECK_CUDA_ERROR(cudaFree(d_acoo_pool));
	CHECK_CUDA_ERROR(cudaFree(d_acoo_pool_index));
	...
	// 计算性能指标
	metrics.speedup = metrics.colored_time_ms / metrics.acoo_time_ms;

	// 清理
	CHECK_CUDA_ERROR(cudaFree(d_global_atomic_counter_acoo));
...
TestConfig runSingleTest(int nx, int ny) {
	...
	double* d_csr_values_acoo = nullptr;
	...
	CHECK_CUDA_ERROR(cudaMalloc(&d_csr_values_acoo, nnz * sizeof(double)));
	...
	PerformanceMetrics metrics = compareAssemblyMethods(
		...
		d_elements,
		d_csr_row_ptr, d_csr_col_ind,
		d_csr_values_acoo, d_csr_values_colored,
	...
	metrics.atomic_time_ms = metrics.atomic_ops_acoo * 0.0001f; // 每个原子操作0.1微秒

   // 确保原子操作时间不超过总时间的90%
	if (metrics.atomic_time_ms > metrics.acoo_time_ms * 0.9f) {
		metrics.atomic_time_ms = metrics.acoo_time_ms * 0.9f;
	}
	...
	CHECK_CUDA_ERROR(cudaFree(d_csr_values_acoo));
...
// ==================== 主函数 ====================
int main() {
	std::cout << "Starting FEM assembly performance comparison (a-COO vs Colored)..." << std::endl;

	// 定义要测试的网格规模
	std::vector<std::pair<int, int>> test_cases = {
		{10, 10},    // 100 elements
		{100, 10},    // ~1000 elements (961)
		{100, 100},  // 10,000 elements
	//	{100, 1000},  // ~100,000 elements (99,856)
	//	{1000, 1000}, // 1,000,000 elements 运行到200万单元的时候超过内存
	};
	...
	for (const auto& result : results) {
		std::cout << std::setw(12) << result.num_elements
			<< std::setw(15) << std::fixed << std::setprecision(2) << result.metrics.acoo_time_ms
			...
			<< std::setw(15) << result.metrics.atomic_ops_acoo
			<< std::endl;
	}
	...
	for (const auto& result : results) {
		double acoo_mem_mb = result.metrics.acoo_mem_usage / (1024.0 * 1024.0);
		...
		double total_mem_mb = acoo_mem_mb + colored_mem_mb + matrix_size_mb;

		std::cout << std::setw(12) << result.num_elements
			<< std::setw(15) << std::fixed << std::setprecision(2) << acoo_mem_mb
	...
	std::cout << std::setw(12) << "Elements"
		<< std::setw(15) << "a-COO GFLOPS"
		<< std::setw(15) << "Colored GFLOPS"
		<< std::setw(15) << "Performance Ratio"
		<< std::endl;

	for (const auto& result : results) {
		double performance_ratio = result.metrics.acoo_flops / result.metrics.colored_flops;

		std::cout << std::setw(12) << result.num_elements
			<< std::setw(15) << std::fixed << std::setprecision(2) << result.metrics.acoo_flops
	...
	// 输出原子操作信息表格
	std::cout << "\n\n========== Atomic Operations in a-COO ==========" << std::endl;
	std::cout << std::setw(12) << "Elements"
	...
	for (const auto& result : results) {
		float fraction = (result.metrics.atomic_time_ms / result.metrics.acoo_time_ms) * 100.0f;

		std::cout << std::setw(12) << result.num_elements
			<< std::setw(20) << result.metrics.atomic_ops_acoo
	...
	perf_file << "Elements,aCOO_Time,Colored_Time,Speedup,Atomic_Ops,"
		<< "aCOO_Mem_MB,Colored_Mem_MB,Matrix_Size_MB,"
		<< "Num_Colors,Max_Color_Size,aCOO_GFLOPS,Colored_GFLOPS\n";

	for (const auto& result : results) {
		perf_file << result.num_elements << ","
			<< result.metrics.acoo_time_ms << ","
			<< result.metrics.colored_time_ms << ","
			<< result.metrics.speedup << ","
			<< result.metrics.atomic_ops_acoo << ","
			<< result.metrics.acoo_mem_usage / (1024.0 * 1024.0) << ","
