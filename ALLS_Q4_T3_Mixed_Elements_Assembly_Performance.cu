...
__device__ void computeElementStiffness(const Element& elem, double* ke) {
	...
	if (elem.type == ELEM_Q4) {
		// Q4: 2x2 = 4 Gauss Points (High Computational Load)
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
		...
		// T3: 1 Gauss Point (Low Computational Load)
...
__global__ void assembleGlobalStiffnessKernelALLS(
	...
	computeElementStiffness(elem, ke); // 计算负载点
...
LoadStats analyzeLoadImbalance(const std::vector<Element>& elements) {
	...
	// Q4使用4个高斯点，T3使用1个。假设基本运算量比例约为 4:1
...
PerformanceMetrics runComparison(
	...
	// A-LLS
	size_t mem_free_start, mem_total;
	...
	cudaEventRecord(stop);
	cudaDeviceSynchronize();
	cudaEventElapsedTime(&metrics.colored_time_ms, start, stop);
